...
function getAverageWordLength(arr){
  let total = 0;
  for (let i = 0; i < arr.length; i++){
    total = total + arr[i].split(' ').length;
  }
...
function getRandomFlavors(arr1, arr2, arr3, arr4){
  ...
  for (let i = 0; i < 31 ; i++) {
    ...
    let flavorIndex = Math.floor(Math.random() * arrays[arrIndex].length);
    randomFlavors.push(arrays[arrIndex][flavorIndex]);
  }
  return randomFlavors;
}
let random = getRandomFlavors(originalFlavors, newFlavors, seasonalFlavors, regionalFlavors)
